<template>
  <div class="Mask">
    ...
    <div v-else>
      ...
      <panel class="panelTopic" width="70%">
        <template #headered>
          <span style="color:gray">{{ userTopic.loginname }} 参与的话题</span>
        </template>
        <template #content>
          <div v-if="!userTopic.recent_replies.length" class="none">
                <span>{{userTopic.loginname}}最近没有参与话题哦，请看看其他的</span>
          </div>
          <div v-else v-for="item in userTopic.recent_replies" :key="item.id">
